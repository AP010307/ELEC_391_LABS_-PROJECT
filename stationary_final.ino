...
const float LEFTTURN      =  2.5;
const float RIGHTTURN     =  2.5;
...
void handleCommand(String c){
  ...
  if(c=="FORWARD"||c=="F"||c=="UP") {
    targetdriveLean = LEAN_SIGN * FWD_LEAN_FWD; targetturnBias = 0;
  } else if(c=="BACKWARD"||c=="B"||c=="DOWN") {
    targetdriveLean = LEAN_SIGN * FWD_LEAN_BACK; targetturnBias = 0;
  } else if(c=="LEFT"||c=="L") {
    targetdriveLean = 0; targetturnBias = -TURN_SIGN * LEFTTURN;
  } else if(c=="RIGHT"||c=="R") {
    targetdriveLean = 0; targetturnBias = TURN_SIGN * RIGHTTURN;
  } else if(c=="STOP"||c=="S"||c=="A") {
    targetdriveLean = 0; targetturnBias = 0; driveLean = 0; turnBias = 0;
  } else {
    targetdriveLean = 0; targetturnBias = 0; driveLean = 0; turnBias = 0;
    resetPID();
  }
...
//6.9, 100, 1.1
  ...
  Serial.println("ARMED + balancing. Cmds: F B L R S");
  lastLoopUs=micros();
}
